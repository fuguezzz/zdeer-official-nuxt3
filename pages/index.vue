<template>
  <main class="h-screen overflow-hidden">
    ...
    <!-- 添加纵向轮播指示器 -->
    <div class="fixed right-5 top-1/2 -translate-y-1/2 flex flex-col gap-3 z-10">
      <button
        v-for="(_, index) in scrollCompoents"
        :key="index"
        @click="goToPage(index + 1)"
        class="w-2 h-2 rounded-full transition-all duration-300 border border-white/50"
        :class="currentPage === index + 1 ? 'bg-white scale-125' : 'bg-white/30 hover:bg-white/50'"
        :aria-label="`跳转到第${index + 1}页`"
      ></button>
    </div>

    <div class="custom-width fixed left-1/2 -translate-x-1/2 bottom-[70px] flex justify-end pointer-events-none">
      <IconUp
        @click="goToPage(1)"
        ...
        :class="[
          ...
          [6].includes(currentPage) ? 'text-black' : 'text-white',
        ]"
      />
    </div>
  </main>
...
const { setTotalPages, goToPage, currentPage, adjustPagePosition } = usePageNavigation();
const innerHeight = useWindowHeight();
import { useGeoJs } from '~/hooks/useGeoJs';
const { geoData, fetchGeoData } = useGeoJs();
